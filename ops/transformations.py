...
class RandomCrop(Crop):
    ...
    def __call__(self, img):
        if type(self.crop_size) is int:
            x_crop = self.crop_size
            y_crop = self.crop_size
        else:
            x_crop, y_crop = self.crop_size

        y, x = img.shape
        x_start = max(random.randint(0, x) // 2 - (x_crop // 2), 0)
        y_start = max(random.randint(0, y) // 2 - (y_crop // 2), 0)
